...
const UpdateSkillShareModal = () => {
  ...
  useEffect(() => {
    if (snap.seletedMealPlanToUpdate) {
      form.setFieldsValue({
        mealDetails: snap.seletedMealPlanToUpdate.mealDetails,
        dietaryPreferences: snap.seletedMealPlanToUpdate.dietaryPreferences,
        ingredients: snap.seletedMealPlanToUpdate.ingredients,
      });

      // Load existing media files
      if (snap.seletedMealPlanToUpdate.mediaUrls && snap.seletedMealPlanToUpdate.mediaUrls.length > 0) {
        const existingFiles = snap.seletedMealPlanToUpdate.mediaUrls.map((url, index) => ({
          uid: `existing-${index}`,
          url: url,
          type: snap.seletedMealPlanToUpdate.mediaTypes ? 
                snap.seletedMealPlanToUpdate.mediaTypes[index] : 
  ...
  }, [snap.seletedMealPlanToUpdate, form]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      await SkillShareService.updateMealPlan(
        snap.seletedMealPlanToUpdate.id,
        {
      ...
      state.seletedMealPlanToUpdate = null;
